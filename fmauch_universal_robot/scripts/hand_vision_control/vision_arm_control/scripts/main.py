...
import time
import math
sys.path.append('//home/lzn/catkin/src/hand_vision_control/python_robot_pub/src')
# import python_robot_pub
...
def main(fps_cap=60, show_fps=True, source=0):
    ...
    J1_Pub = RosPub(node_name="joint_group_vel_publisher", topic="/robot1/joint_group_vel_controller/command", data="Float64", queue=10, signal=True)
    time.sleep(0.5)
    J2_Pub = RosPub(node_name="vel_joint_traj_publisher", topic="/robot1/vel_joint_traj_controller/command", data="Float64", queue=10, signal=True)
    ...
    while True:  # infinite loop for webcam video capture
        ...
        if delta_time >= time_lim:  # if the time passed is bigger or equal than the frame time, process the frame
            ...
            if len(hand_lmlist) > 0:
                ...
                J1_Pub.publish_once(message=3.0 - 0.3*(aperture/10)) # message=3.0 - 0.3*(aperture/10)
                J2_Pub.publish_once(message=3.0 - 0.3*(aperture/10)) # message=3.0 - 0.3*(aperture/10)
