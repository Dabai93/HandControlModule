...
class HandDetector():
    ...
    def fingersUp(self):
        fingers = []
        # 大拇指
        if self.lm_list[self.tipIds[0]][1] > self.lm_list[self.tipIds[0] - 1][1] + 10:
    ...
    def findHandAperture(self, frame, verbose=False, show_aperture=True, aperture_range: list = [0.4, 1.7]):
        ...
        if show_aperture:
            frame = cv2.line(frame, tuple(tips_midpoint_array.astype(int)),
                             tuple(lower_palm_midpoint_array.astype(int)), (0, 255, 255), 3)
            cv2.circle(frame, (tuple(tips_midpoint_array.astype(int))[0], tuple(tips_midpoint_array.astype(int))[1]), 8, (255, 0, 0), -1)
            cv2.circle(frame, (tuple(lower_palm_midpoint_array.astype(int))[0], tuple(lower_palm_midpoint_array.astype(int))[1]), 8, (255, 0, 0), -1)                      
